        ...
        loop {
            ...
            let response_body = match &request.command {
                ...
                Command::SetFunctionBreakpoints(_) => {
                     Some(ResponseBody::SetFunctionBreakpoints(dap::responses::SetFunctionBreakpointsResponse {
                        breakpoints: vec![]
                    }))
                }
                ...
                Command::StackTrace(_) => {
                    ...
                    let (source, line, name) = if let Some(loc) = source_loc {
                        let source = Some(Source {
                            name: Some(std::path::Path::new(&loc.file)
                                .file_name()
                                .and_then(|n| n.to_str())
                                .unwrap_or(&loc.file)
                                .to_string()),
                        ...
                        (source, loc.line.map(|l| l as i64), loc.function.unwrap_or_else(|| "main".to_string()))
                    } else {
                        (None, Some(0), "unknown".to_string())
                    };

                    Some(ResponseBody::StackTrace(dap::responses::StackTraceResponse {
